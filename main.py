...
imgModeList = [
    ...
    for path in sorted(os.listdir(folderModePath), key=sort_key)]
...
last_id = None
last_emotion_times = {}
emotion_interval = timedelta(seconds=5)
check_numfaces = False
RELAY_PIN = 17
...
while True:
    ...
    imgface = cv2.resize(img, (0, 0), None, 0.30, 0.30)
    ...
    for face in faces:
        ...
        if avg_ear_history > EAR_THRESHOLD and check_numfaces is True:
            open_eye_frames += 1
        else:
        ...
        if faceCurFrame:
            ...
            if counter != 0:
                if counter == 1:
                    ...
                    current_time = datetime.now()
                    if id not in last_emotion_times or current_time - last_emotion_times[id] > emotion_interval:
                        emotions = emotion_detector.detect_emotions(imgface)
                        last_emotion_times[id] = current_time
